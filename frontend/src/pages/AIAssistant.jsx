      ...
      <div className="mx-auto max-w-4xl w-full flex-1 flex flex-col space-y-6">
        ...
        <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2 text-xs font-semibold text-slate-500 bg-slate-50 px-3 py-2 rounded-xl border border-slate-200 self-start sm:self-auto">
            ...
            <span>Municipal Knowledge Base v1.0</span>
          </div>
        </div>
          ...
          <div className="flex flex-wrap gap-2">
            {QUICK_PROMPTS.map((prompt, idx) => (
              <button
                ...
                disabled={loading}
                className="text-xs font-semibold text-slate-700 bg-white hover:bg-blue-50 hover:text-blue-700 border border-slate-200/80 rounded-xl px-3.5 py-2 transition-all shadow-sm disabled:opacity-50 text-left"
              >
                {prompt}
              </button>
            ))}
        ...
        <div className="flex-1 bg-white rounded-3xl border border-slate-200 shadow-lg p-4 sm:p-6 min-h-[420px] max-h-[550px] overflow-y-auto flex flex-col space-y-4">
          {messages.map((msg) => (
            <div
              key={msg.id}
                ...
                {msg.sender === "bot" && msg.sources && msg.sources.length > 0 && (
                  <div className="pt-2 border-t border-slate-200/60 flex flex-wrap items-center gap-1.5">
                    ...
                    {msg.sources.map((src, i) => (
                      <span
                        key={i}
                        className="px-2 py-0.5 rounded-full text-[10px] font-semibold bg-white text-slate-700 border border-slate-200"
                      >
                        {src}
                      </span>
                    ))}
          ...
          {loading && (
            <div className="flex items-center gap-3">
              <div className="w-9 h-9 rounded-2xl bg-gradient-to-br from-indigo-500 to-blue-600 text-white flex items-center justify-center shrink-0 shadow-sm">
                <Bot className="h-5 w-5" />
              </div>
        ...
        {/* Input Bar */}
        <div className="bg-white rounded-2xl border border-slate-200 shadow-lg p-2.5 flex items-center gap-2">
          <textarea
            rows={1}
            value={inputQuestion}
            ...
            className="w-full px-3 py-2 text-sm border-0 focus:ring-0 focus:outline-none resize-none bg-transparent text-slate-900 placeholder-slate-400"
          />
          <button
            type="button"
            onClick={() => handleSend()}
            disabled={loading || !inputQuestion.trim()}
            className="flex items-center justify-center w-11 h-11 rounded-xl bg-blue-600 hover:bg-blue-700 text-white shadow-md transition-colors disabled:opacity-50 shrink-0"
          >
