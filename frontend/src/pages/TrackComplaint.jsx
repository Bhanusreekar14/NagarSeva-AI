      ...
      <div className="mx-auto max-w-4xl space-y-8">
        ...
        <div className="text-center">
          ...
          <p className="mt-2 text-slate-600 text-sm sm:text-base">
            Track real-time progress, department updates, and resolution timeline
          </p>
        </div>
        ...
        <form
          ...
          className="mx-auto max-w-2xl bg-white p-2.5 rounded-2xl shadow-lg border border-slate-200 flex items-center gap-2"
        >
          <div className="relative flex-1">
            ...
            <input
              ...
              className="block w-full pl-11 pr-4 py-3 text-sm border-0 bg-transparent text-slate-900 placeholder-slate-400 focus:ring-0 focus:outline-none uppercase font-semibold"
            />
          </div>
          <button
            type="submit"
            disabled={loading}
            className="flex items-center gap-2 px-6 py-3 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-bold text-sm shadow-md transition-all disabled:opacity-50 shrink-0"
          >
            {loading ? (
        ...
        {loading && (
          <div className="p-12 text-center bg-white rounded-3xl border border-slate-100 shadow-sm space-y-3">
            ...
            <p className="text-sm font-semibold text-slate-600">Retrieving complaint details & timeline...</p>
          </div>
        )}
            ...
            <div className="bg-white rounded-3xl border border-slate-200 p-6 sm:p-8 shadow-xl space-y-6">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-slate-100">
                ...
                <div className="flex items-center gap-3">
                  <span
                    className={`px-4 py-2 rounded-2xl border font-bold text-sm shadow-sm ${getStatusColor(
                      details.status
                    )}`}
                  >
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                ...
                <SpecItem
                  label="Severity"
                  value={
                    <span className={`px-2.5 py-0.5 rounded-full text-xs border font-semibold ${getSeverityBadge(details.severity)}`}>
                      {details.severity}
                    </span>
                  }
                  icon={AlertTriangle}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
                <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200/80 flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-xs font-semibold text-slate-500">Assigned Department</p>
                    <p className="text-sm font-bold text-slate-800">{details.department}</p>
                  </div>
                </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {details.attachments.map((att, i) => (
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-slate-100 hover:bg-blue-50 hover:text-blue-700 text-xs font-semibold text-slate-700 transition-colors border border-slate-200"
                      >
                    ...
                    return (
                      <div
                        ...
                        className={`flex md:flex-col items-center gap-4 md:gap-2 text-left md:text-center p-3 rounded-2xl border md:border-0 ${
                          isCurrent
                            ? "bg-blue-50/80 border-blue-200 md:bg-transparent"
                            : "border-slate-100 bg-white md:bg-transparent"
                        }`}
                      >
                        <div
                          className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm transition-all shadow-md shrink-0 ${
                            isCurrent
                        ...
                        <div>
                          <p className={`text-xs font-bold ${isCurrent ? "text-blue-700 font-extrabold" : isCompleted ? "text-slate-900" : "text-slate-400"}`}>
                            {step.label}
                          </p>
                      ...
                      <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200/80 space-y-1">
                        ...
                        {event.remarks && (
                          <p className="text-xs text-slate-600 pt-1 border-t border-slate-200/50 mt-2">
                            <span className="font-semibold text-slate-700">Remarks:</span> {event.remarks}
                          </p>
                        )}
                      </div>
                    </div>
